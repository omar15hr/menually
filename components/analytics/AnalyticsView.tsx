...
export default function AnalyticsView({ initialData }: AnalyticsViewProps) {
  ...
  const generalMetrics = [
    {
      ...
      trendType:
        data.qrScans.totalScans > 0
          ? ("positive" as const)
          : ("negative" as const),
      footerLabel: "Alta hoy",
      footerValue:
        data.qrScans.totalScans > 0 ? "Entre 13:00 - 15:00" : "Sin datos",
      info: false,
    },
    {
    ...
    },
    {
      ...
      trendType:
        data.timeMetrics.avgTimeOnPage && data.timeMetrics.avgTimeOnPage > 30
          ? ("positive" as const)
          : ("neutral" as const),
  ...
  const getHeatmapData = () => {
    const heatmap2d: number[][] = Array.from({ length: 7 }, () =>
  ...
  const performanceCategorias = data.categoryPerformance
    .slice(0, 4)
    .map((cat) => ({
      name: cat.name,
  ...
  const performanceProductos = data.productPerformance
    .slice(0, 4)
    .map((prod) => ({
      name: prod.name,
  ...
  const leastViewedCategory =
    data.qrScans.leastViewedCategory?.name ?? "Sin datos";

  // Most viewed product name
  const mostViewedProduct = data.productPerformance[0]?.name ?? "Sin datos";
  const leastViewedProduct =
    data.productPerformance.length > 0
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {generalMetrics.map((metric, idx) => (
                <Card
                  key={idx}
                  className="rounded-2xl border-gray-200 bg-[#FBFBFA]"
                >
                  ...
                  <CardContent>
                    <div className="flex justify-between items-end mb-6">
                      <span
                        className={`text-xs font-semibold px-2 py-1 rounded-md ${metric.trendType === "positive" ? "bg-green-100 text-green-700" : metric.trendType === "negative" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-700"}`}
                      >
                        {metric.trend}
                      </span>
                            ...
                            <div className="flex-1 flex gap-1">
                              {heatmap2d[dIdx].map((val, hIdx) => {
                                ...
                                return (
                                  <div
                                    key={hIdx}
                                    className={`h-5 flex-1 rounded-xs ${colors[colorIdx]}`}
                                    title={`${val} escaneos`}
                                  />
                                );
                              })}
                  ...
                  <CardHeader className="flex flex-row items-center justify-between pb-8">
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      <div className="flex items-center gap-1">
                        <div className="w-3 h-1 bg-(--color-current) rounded-full" />{" "}
                        {periodLabels[period]}
                      </div>
                      <div className="flex items-center gap-1">
                        <div className="w-3 h-1 bg-(--color-current) rounded-full" />{" "}
                  ...
                  <Card className="rounded-2xl shadow-sm border-gray-200">
                    ...
                    <CardContent>
                      <div className="space-y-5">
                        {performanceCategorias.length > 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <CardContent className="p-5 flex flex-col h-full justify-between">
                      <div>
                        ...
                        <div className="flex justify-between items-center mb-6">
                          <span className="px-3 py-1 bg-gray-50 text-gray-700 text-xs font-semibold rounded-full border border-gray-100">
                            Sin variación
                          </span>
                          <span className="text-5xl font-black text-gray-900">
                            {formatNumber(data.shareMetrics.totalShares)}
                          </span>
                        </div>
                      </div>
                      <div className="border-t border-gray-100 pt-3">
                        <p className="text-xs text-gray-500 font-medium">
                          Día con más escaneos
                        </p>
                        <p className="text-sm font-bold text-gray-900">
                          {mostSharedDayName}
                        </p>
                  ...
                  <Card className="rounded-2xl shadow-sm border-gray-200">
                    <CardContent className="p-5 flex flex-col h-full justify-between">
                      <div>
                        ...
                        <div className="flex justify-between items-center mb-6">
                          <span className="px-3 py-1 bg-[#dcfce7] text-green-800 text-xs font-semibold rounded-full">
                            +{formatSeconds(data.timeMetrics.avgTimePerProduct)}{" "}
                            esta semana
                          </span>
                          <span className="text-5xl font-black text-gray-900">
                            {data.timeMetrics.avgTimePerProduct
                              ? Math.round(data.timeMetrics.avgTimePerProduct)
                              : 0}
                      ...
                      <div className="border-t border-gray-100 pt-3">
                        <p className="text-xs text-gray-500 font-medium">
                          Producto con más permanencia
                        </p>
                        <p className="text-sm font-bold text-gray-900">
                          {data.timeMetrics.mostAttentionProduct?.name ??
                            "Sin datos"}
                        </p>
                      </div>
                    </CardContent>
                        ...
                        >
                          <div className="flex items-center gap-2 flex-wrap text-xs font-medium">
                            {pattern.path.map((step, j) => (
                              <React.Fragment key={j}>
                                <span className="bg-gray-100 px-3 py-1.5 rounded-lg border">
                                  {step}
                                </span>
                                {j < pattern.path.length - 1 && (
                          ...
                          <div className="flex items-center gap-3 w-32 justify-end">
                            <div className="h-1.5 flex-1 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                className="h-full bg-[#355e16] rounded-full"
